...
export default {
  ...
  methods: {
    ...mapActions("wallet", ["createTransaction"]),
    async handleCreateTransaction() {
      ...
      try {
        if (senderInput.length < 40) {
          throw new Error("Please select a valid source address");
        }
        ...
        await apiRequest.transactions.create(
          senderInput,
          receiverInput,
          amountInput
        );
  ...
  watch: {
    show() {
      this.sendLoading = this.fullscreen = false;

      this.senderInput = this.receiverInput = "";
    },
  },
};
</script>
